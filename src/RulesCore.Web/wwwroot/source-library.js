...
function applyLibraryRoute(app, route) {
    ...
    const publicationId = segments[1] ?? null;
    if (!publicationId) {
        app.activeView = "library";
        app.libraryRoute = { kind: "index", query: query.get("q") ?? "", page: pageNumber(query) };
        return true;
...
async function renderLibraryIndex(app, container, route) {
    ...
    const needle = String(route.query ?? "").trim().toLowerCase();
    const visible = needle
        ? publications.filter(value => [value.displayName, value.workDisplayName, value.editionDisplayName, value.provider]
            .some(candidate => String(candidate ?? "").toLowerCase().includes(needle)))
        : publications;

    if (!visible.length) {
        container.append(alertNode("secondary", "No accessible source publications match this search."));
        return;
    }

    const grid = element("div", { className: "rules-core-publication-grid" });
    for (const publication of visible) grid.append(publicationCard(app, publication));
    container.append(grid);
}

function publicationCard(app, publication) {
    const href = app.toolHref(publication.browserLink.toolRelativePath);
    const card = element("article", { className: "card card-body rules-core-publication-card" },
...
function categoryPreview(categories = []) {
    ...
    for (const category of categories.slice(0, 6)) {
        row.append(element("span", { className: "rules-core-category-chip", text: `${category.displayName} ${category.count}` }));
    }
...
async function renderPublication(app, container, route) {
    ...
    const categories = element("section", { className: "mb-4" },
        element("h3", { className: "h5 mb-3", text: "Browse by category" }));
...
async function renderCollection(app, container, route) {
    ...
    const title = category?.displayName ?? humanize(route.entityType);
    container.append(breadcrumbs(app, [
        ["Rules Library", "/library"],
...
async function renderEntityList(app, container, { publicationId, entityType, query, page, title, basePath }) {
    ...
    const search = element("input", {
        type: "search",
        className: "form-control",
        placeholder: entityType ? `Search ${humanize(entityType).toLowerCase()}` : "Search this publication",
        value: query ?? ""
    });
    const form = element("form", { className: "rules-core-library-search mb-3" }, search,
        element("button", { type: "submit", className: "btn btn-primary", text: "Search" }));
    ...
    for (const entry of entries) {
        ...
        list.append(element("a", {
            ...
            element("span", { className: "rules-core-entry-main" },
                ...
                element("small", { text: entry.sourceCode })),
    ...
    container.append(paginationControls({
        ...
        onPage: async nextPage => {
            await app.navigateToolRoute(withQuery(basePath, {
                q: query || null,
                page: nextPage + 1
            }));
        }
...
async function renderEntity(app, container, route) {
    ...
    container.append(breadcrumbs(app, [
        ["Rules Library", "/library"],
        [publication.displayName, publication.browserLink.toolRelativePath],
        [category?.displayName ?? humanize(entity.entityType), category?.browserLink?.toolRelativePath ?? null],
        [entity.name, null]
    ]));
