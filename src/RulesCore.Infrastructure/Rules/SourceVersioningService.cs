...
public sealed class SourceVersioningService(RulesCoreDbContext dbContext) : ISourceVersioningService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            var confidence = ScoreCandidate(
                source, sourceLatest, sourceConcepts, sourceMetadata,
                candidate, Latest(candidate), candidateConcepts, candidateMetadata,
                lineage, reasons);
            if (confidence < 30) continue;
            scored.Add(new SourceVersionCandidateView(
                ToEntityView(candidate, candidateConcepts, candidateMetadata), confidence, reasons));
        }

        return new SourceVersionDetectionView(
            sourceView,
            scored.OrderByDescending(value => value.Confidence)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var accessibleCount = await AccessibleSources(actor).CountAsync(
        ...
        var result = await SourceFrameworkStore.CreateLineageAsync(
            dbContext, request.FromSourceEntityId, request.ToSourceEntityId, kind, note, actor, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existing = await SourceFrameworkStore.GetLineageByIdAsync(dbContext, lineageId, cancellationToken);
        ...
        var result = await SourceFrameworkStore.VoidLineageAsync(
            dbContext, lineageId, reason, actor, cancellationToken);
        return result is null ? null : new SourceLineageMutationView(ToView(result.Value.Lineage), result.Value.Changed);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var lineage = await SourceFrameworkStore.GetLineageForSourcesAsync(
            dbContext, accessibleIds, includeVoided: false, cancellationToken);
        return lineage
        ...
        dbContext.SourceEntities
            ...
            .Where(value => value.Revisions.Any()
                && (value.SourcePackage.IsPublic
                    || value.SourcePackage.UserGrants.Any(grant => grant.UserId == userId)));

    private static SourceVersionEntityView ToEntityView(
        SourceEntity entity,
        ...
        List<string> reasons)
    {
        ...
        if (directLineage) reasons.Add("Confirmed source lineage connects these implementations.");
    ...
    private static bool ContainsExplicitReference(string rawJson, string targetName, string targetSourceCode)
    {
        ...
        if (document.RootElement.ValueKind == JsonValueKind.Object)
        {
            ...
            {
                if (ReferenceProperties.Contains(property.Name)) CollectStrings(property.Value, values);
            }
        }
        ...
        return values.Any(value =>
        {
            ...
            if (!normalized.Contains(normalizedTarget, StringComparison.Ordinal)) return false;
    ...
    private static void CollectStrings(JsonElement element, ICollection<string> values)
    {
        switch (element.ValueKind)
        {
            ...
            case JsonValueKind.Array:
                foreach (var child in element.EnumerateArray()) CollectStrings(child, values);
                break;
    ...
    private static void CollectContentTokens(JsonElement element, ISet<string> tokens, string? propertyName)
    {
        ...
        switch (element.ValueKind)
        {
            case JsonValueKind.String:
                ...
                {
                    if (token.Length >= 4 && !ContentStopWords.Contains(token)) tokens.Add(token);
                }
                break;
            case JsonValueKind.Array:
                foreach (var child in element.EnumerateArray()) CollectContentTokens(child, tokens, propertyName);
                break;
            case JsonValueKind.Object:
                foreach (var property in element.EnumerateObject()) CollectContentTokens(property.Value, tokens, property.Name);
                break;
        }
    }
    ...
    private static SourceLineageView ToView(StoredSourceLineage lineage) =>
        new(lineage.Id, lineage.FromSourceEntityId, lineage.ToSourceEntityId, lineage.RelationshipKind,
            lineage.Note, lineage.CreatedByUserId, lineage.CreatedAt, lineage.VoidedAt is not null,
            lineage.VoidReason, lineage.VoidedByUserId, lineage.VoidedAt);

    private static string RequireLineageKind(string value)
    {
        if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException("Lineage relationship kind can not be blank.", nameof(value));
        var normalized = value.Trim().ToLowerInvariant();
        if (!SourceLineageKinds.All.Contains(normalized))
        {
            throw new ArgumentException($"Unsupported lineage relationship kind '{value}'.", nameof(value));
        }
        return normalized;
    }
...
{
    public static async Task<CanonicalPublicationMetadata?> ReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return values.OrderByDescending(value => value.PublicationDate ?? DateOnly.MinValue).First();
        }
        catch (DbException)
        {
            return null;
    ...
    : IRuleConsolidationService
{
    public async Task<RuleConsolidationView?> GetAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (ruleConceptId == Guid.Empty) throw new ArgumentException("Rule concept ID can not be empty.", nameof(ruleConceptId));
        ...
        if (detail.LatestDecision is not null)
        {
            var stored = await SourceFrameworkStore.GetDecisionContributionsAsync(
                dbContext, detail.LatestDecision.Id, cancellationToken);
            foreach (var contribution in stored)
            {
                if (!revisionLookup.TryGetValue(contribution.SourceEntityRevisionId, out var context)) continue;
                contributions.Add(new RuleConsolidationContributionView(
                    ...
                    context.Source.Id,
                    context.Source.Name,
                    context.Source.SourceCode ?? string.Empty,
        ...
        return new RuleConsolidationView(
            detail.Concept, sourceViews, detail.RestrictedBindingCount, lineage, detail.LatestDecision, contributions);
    }
}
