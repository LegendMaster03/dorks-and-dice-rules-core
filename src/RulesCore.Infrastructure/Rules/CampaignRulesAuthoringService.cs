    ...
    : ICampaignRulesAuthoringService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (latestPublished is not null)
        {
            var publishedEntries = await dbContext.CampaignRulesetRevisionEntries
                ...
                .Select(value => new { value.RuleConceptId, value.CampaignRuleDecisionId })
                .ToArrayAsync(cancellationToken);
            publishedDecisionIds = publishedEntries.ToDictionary(value => value.RuleConceptId, value => value.CampaignRuleDecisionId);
        ...
        if (selectedBaseline is null)
        {
            return new CampaignRulesAuthoringOverviewView(
                campaignId, null, publishedSummary, false, false, 0, 0, 0, []);
        }
        ...
        var baselineConceptIds = baselineEntries.Select(value => value.RuleConceptId).ToArray();
        var allCampaignDecisions = baselineConceptIds.Length == 0
            ? []
            : await dbContext.CampaignRuleDecisions
                .AsNoTracking()
                .Where(value => value.CampaignId == campaignId && baselineConceptIds.Contains(value.RuleConceptId))
        ...
        var conceptViews = baselineEntries.Select(entry =>
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var selectedBaseline = await dbContext.CampaignRulesetSelections
            ...
            .OrderByDescending(value => value.SelectionNumber)
            .FirstOrDefaultAsync(cancellationToken);
        if (selectedBaseline is null) return null;
        ...
        var baselineEntry = await dbContext.RulesetRevisionEntries
            ...
            .SingleOrDefaultAsync(
                value => value.RulesetRevisionId == selectedBaseline.RulesetRevisionId && value.RuleConceptId == ruleConceptId,
                cancellationToken);
        if (baselineEntry is null) return null;
        ...
        if (latestPublished is not null)
        {
            publishedCampaignDecisionId = await dbContext.CampaignRulesetRevisionEntries
                .AsNoTracking()
                .Where(value => value.CampaignRulesetRevisionId == latestPublished.Id && value.RuleConceptId == ruleConceptId)
    ...
    private static CampaignRulesetSelectionAuthoringSummaryView ToSummary(CampaignRulesetSelection selection) =>
        new(selection.Id, selection.SelectionNumber, selection.RulesetRevisionId, selection.RulesetRevision.RevisionNumber,
            selection.RulesetRevision.Fingerprint, selection.SelectedByUserId, selection.SelectedAt);

    private static PublishedCampaignRulesetAuthoringSummaryView ToSummary(CampaignRulesetRevision revision, int entryCount) =>
        new(revision.Id, revision.RevisionNumber, revision.Fingerprint, revision.BaselineSelectionId,
            revision.BaselineSelection.RulesetRevisionId, revision.BaselineSelection.RulesetRevision.RevisionNumber,
            revision.BaselineSelection.RulesetRevision.Fingerprint, revision.PublishedByUserId, revision.PublishedAt, entryCount);
    ...
    private static GlobalRuleDecisionView ToView(GlobalRuleDecision decision)
    {
        ...
        return new GlobalRuleDecisionView(
            decision.Id, decision.RuleConceptId, decision.DecisionNumber, decision.DecisionKind,
            decision.SelectedSourceEntityRevisionId, decision.SelectedSourceEntityRevision.SourceEntityId,
            decision.SelectedSourceEntityRevision.RevisionNumber, decision.SelectedSourceEntityRevision.Fingerprint,
            decision.PatchFingerprint, mergePatch, structuredPatch, decision.Note, decision.CreatedByUserId, decision.CreatedAt);
    }
    ...
    private static CampaignRuleDecisionView ToView(CampaignRuleDecision decision)
    {
        ...
        return new CampaignRuleDecisionView(
            decision.Id, decision.CampaignId, decision.RuleConceptId, decision.DecisionNumber, decision.DecisionKind,
            decision.SelectedSourceEntityRevisionId, decision.PatchFingerprint, mergePatch, structuredPatch,
            decision.Note, decision.CreatedByUserId, decision.CreatedAt, Created: false);
    }

    private static GlobalRuleAuthoringSourceView ToSourceView(Domain.Sources.SourceEntity source)
    {
        ...
        return new GlobalRuleAuthoringSourceView(
            ...
            source.Revisions
                ...
                .Select(value => new GlobalRuleAuthoringSourceRevisionView(
                    value.Id, value.RevisionNumber, value.Fingerprint, value.ImportedAt))
                .ToArray());
    }
