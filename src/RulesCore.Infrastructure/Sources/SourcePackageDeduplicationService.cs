...
/// <summary>
/// Consolidates legacy per-user source packages that contain the exact same physical
/// representation set. Current-user registrations and grants are moved to one shared
/// package. A duplicate that is referenced by non-cascading Rules history is retained
/// rather than rewriting authored history implicitly.
/// </summary>
        ...
        try
        {
            if (await RelationExistsAsync(connection, "user_source_grant", cancellationToken, transaction))
            {
                ...
                grants.CommandText = """
                    ...
                    SELECT
                        gen_random_uuid(), @keeper_package_id, duplicate.user_id, duplicate.granted_at
                    FROM user_source_grant duplicate
                    WHERE duplicate.source_package_id = @duplicate_package_id
                    ON CONFLICT (user_id, source_package_id) DO NOTHING;
                    """;
                AddParameter(grants, "@keeper_package_id", keeperPackageId);
                AddParameter(grants, "@duplicate_package_id", duplicatePackageId);
