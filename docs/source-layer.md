...
GitHub tree imports enumerate compatible files and preserve each fetched file as its own package-scoped source representation. When multiple accounts import byte-identical files, those representation rows reference one shared content blob while their grants and provenance remain independent. Large Web adds are resumable across service restarts: each completed representation remains committed, interrupted running jobs are requeued, and retry reuses those deterministic package/origin representations instead of deleting the partial package. Explicit maintenance can still clean an abandoned incomplete package.
