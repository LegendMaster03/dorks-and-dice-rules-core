...
Source Layer import does not normally make source content an effective table rule. The exact checked-in reviewed SRD representations remain a narrow bootstrap exception for competencies.
...
Bootstrap checks the complete synchronization result before global publication. Re-running bootstrap remains idempotent through stable concept keys, canonical binding uniqueness, guarded initial decisions, and ruleset fingerprints.
